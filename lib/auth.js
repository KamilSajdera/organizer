...
export async function loginUser(prevState, formData) {
  const enteredLogin = formData.get("email");
  const enteredPassword = formData.get("password");
  const isRememberChecked = formData.get("remember") === "on";

  if (isEmptyField(enteredLogin)) return { message: "Login field is empty!" };
  if (isEmptyField(enteredPassword))
    return { message: "Password field is empty!" };

  const userExist = await isUserExists(enteredLogin);
  if (!userExist)
    return {
  ...
  try {
    ...
    const usersCollection = db.collection("users");
    const getUserData = await usersCollection.findOne({
      email: enteredLogin,
    });

    const validPassword = verifyPassword(getUserData.password, enteredPassword);

    if (!validPassword) return { message: "Bad password! Try again." };

    await createSession(getUserData._id.toString(), isRememberChecked);
    revalidatePath("/");
  } catch (error) {
