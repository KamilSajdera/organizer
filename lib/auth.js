...
export default async function registerUser(prevState, formData) {
  ...
  const client = await MongoClient.connect(process.env.NEXT_PUBLIC_MONGODB_USERS_DATA);
  const db = client.db();

  const meetupsCollection = db.collection("users");
  const result = await meetupsCollection.insertOne(userData);

  client.close();

  return { success_message: "You've sign Up successfully! Now you can Sign In."}
}
