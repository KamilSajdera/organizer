...
export async function updateCategory(id, categoryName) {
  const client = await MongoClient.connect(process.env.NEXT_PUBLIC_MONGODB_ACTIVITIES_DATA);
  const db = client.db();
  ...
  const tasksCollection = db.collection("tasks");
  const updateResult = await tasksCollection.updateOne({_id:  new ObjectId(id)}, updateData)
